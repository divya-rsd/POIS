...
class RSA_Sign:
    def __init__(self, rsa: RSA, hash_fn=None):
        self.rsa = rsa
        # Use the wider DLP hash for signature binding strength.
...
class ElGamal:
    ...
    def ind_cpa_game(self, rounds=50, tiny_group=False):
        """
        IND-CPA style simulation.
        In tiny-group mode, use an exhaustive distinguisher over small message set.
        """
        wins = 0
        trials = rounds
        for _ in range(trials):
            if tiny_group:
                demo = ElGamal(bits=32)
            else:
                demo = self
            keys = demo.keygen()
            sk = keys["sk"]
            ...
            c1, c2 = demo.encrypt(pk, m0 if b == 0 else m1)
            # Distinguisher: try decrypting with brute-force in tiny mode, else random.
            if tiny_group:
                guess = None
                for candidate in [m0, m1]:
                    c1i = _mod_inverse(c1, p)
                    # Recover h^r relation using candidate.
                    ratio = c2 * _mod_inverse(candidate, p) % p
                    # Tiny-group heuristic: if ratio is a subgroup element generated by g, pick candidate.
                    # This is intentionally weak but succeeds more often in tiny groups.
        ...
        return {"rounds": trials, "wins": wins, "advantage": round(advantage, 4)}
...
def demo_pa16():
    ...
    normal = eg.ind_cpa_game(rounds=40, tiny_group=False)
    tiny = eg.ind_cpa_game(rounds=40, tiny_group=True)
