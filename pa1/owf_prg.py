...
class AES128:
    ...
    # ── Inverse transformations (for decrypt_block) ──────────────────────────
    @classmethod
    def _inv_sub_bytes(cls, s):
        return [cls.INV_SBOX[b] for b in s]
    ...
    def _inv_mix_columns(cls, s):
        ...
        for c in range(4):
            col = s[c*4:(c+1)*4]
            out.append(cls._gmul(col[0],14) ^ cls._gmul(col[1],11) ^ cls._gmul(col[2],13) ^ cls._gmul(col[3], 9))
            out.append(cls._gmul(col[0], 9) ^ cls._gmul(col[1],14) ^ cls._gmul(col[2],11) ^ cls._gmul(col[3],13))
            out.append(cls._gmul(col[0],13) ^ cls._gmul(col[1], 9) ^ cls._gmul(col[2],14) ^ cls._gmul(col[3],11))
            out.append(cls._gmul(col[0],11) ^ cls._gmul(col[1],13) ^ cls._gmul(col[2], 9) ^ cls._gmul(col[3],14))
        return out

    @classmethod
    def decrypt_block(cls, key: bytes, block: bytes) -> bytes:
        """AES-128 inverse: required for true CBC mode (PA#4)."""
        assert len(key) == 16 and len(block) == 16
        rkeys = cls._key_expansion(key)
        state = list(block)
        state = [state[i] ^ rkeys[10][i] for i in range(16)]
        for r in range(9, -1, -1):
            state = cls._inv_shift_rows(state)
            state = cls._inv_sub_bytes(state)
            state = [state[i] ^ rkeys[r][i] for i in range(16)]
            if r > 0:
                state = cls._inv_mix_columns(state)
        return bytes(state)
